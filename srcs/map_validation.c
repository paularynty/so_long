...
/*   Updated: 2024/09/06 13:25:54 by prynty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_walls_x(t_game *game, char *map)
{
	...
	while (i < game->map_width)
	{
		if (game->map[0][i] != '1')
		{
			print_error("Verify map walls (hint: top row)");
			free(map);
			free_game(game);
			return (1);
		}
		...
		{
			print_error("Verify map walls (hint: bottom row)");
			free(map);
			free_game(game);
			return (1);
		}
...
int	check_walls_y(t_game *game, char *map)
{
	...
	while (i < game->map_height)
	{
		if (game->map[i][0] != '1')
		{
			print_error("Verify map walls (hint: left column)");
			free(map);
			free_game(game);
			return (1);
		}
